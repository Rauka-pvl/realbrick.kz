          ...
          @forelse($products as $product)
            <div class="group relative overflow-hidden rounded-3xl border border-white/10 bg-charcoal shadow-lg">
              <a href="{{ route('catalog.product', ['slug' => $product['slug']]) }}">
                ...
                <div class="absolute bottom-0 left-0 right-0 px-6 py-5 pr-24">
                  ...
                  @if(!empty($product['price_value']))
                    <div class="mt-1 text-sm font-semibold text-gold">{{ number_format((float) $product['price_value'], 0, '.', ' ') }} $</div>
                  @endif
              ...
              <form method="POST" action="{{ route('cart.add') }}" class="absolute bottom-4 right-4 z-20">
                ...
                <input type="hidden" name="price_currency" value="{{ $product['price_currency'] ?? 'USD' }}">
